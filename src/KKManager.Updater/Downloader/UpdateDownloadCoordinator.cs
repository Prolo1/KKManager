	...
	internal class UpdateDownloadCoordinator : IDisposable
	{
		...
		public static UpdateDownloadCoordinator Create(IEnumerable<UpdateTask> updateTasks, CancellationToken cancellationToken)
		{
			...
			var sortedUpdateItemInfos = updateItemInfos
				...
				//Make sure all the files are in order of size or large files that will fail 
				.ThenBy(sources => sources.TotalSize)
		...
		private void UpdateThread(DownloadSourceInfo updateSource)
		{
			...
			try
			{
				...
				while(!_cancellationToken.IsCancellationRequested)
				{
					...
					lock(_updateItems)
					{
						FileSize getCurrentDLSize() =>
							new FileSize(_updateItems.Where((k) =>
							k.Status == UpdateDownloadStatus.Downloading).
							Sum((v) => v.TotalSize.GetKbSize()));
					...
					catch(Exception e)
					{
						...
						lock(_updateItems)
						{
							...
							currentDownloadItem.FinishPercent = 0;
							currentDownloadItem.Order = _updateItems.Last().Order + 1;
							currentDownloadItem.Status = UpdateDownloadStatus.Waiting;
							_updateItems.Sort((a, b) => a.Order.CompareTo(b.Order));
						}
